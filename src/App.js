import React, { useEffect, useState } from 'react';
import './index.css';
import './App.css';
import Sport from './Companent/Sport';
import Navbar from './Companent/Navbar';
import Education from './Companent/Education';
import Car from './Companent/Car';
import { Routes, Route } from 'react-router-dom';


function App() {
  const [data, setData] = useState([]);
  const [inputValue, setInputValue] = useState('');

  const sabmited = (e) => {
    e.preventDefault();
      fetch(
        `https://newsapi.org/v2/everything?q=${inputValue == '' ? 'world' : inputValue}&from=2023-03-07&sortBy=publishedAt&apiKey=fdb8c823b0eb42c1bfe0f5b9f97ac325`)
        .then((res) => res.json())
        .then((data)=> setData(data.articles));
  }
  return (
    <div className='app'>
      <Navbar/>

      <Routes>
        <Route path='/car' element={ < Car /> } />
        <Route path='/education' element={ < Education /> } />
        <Route path='/sport' element={ < Sport /> } />
      </Routes>
      <hr/>
      <form onSubmit={sabmited}>
        <input type='text' value={inputValue} onChange={e=> setInputValue(e.target.value)} placeholder='Any...'/>
        <button className='btn'>Search</button>
      </form>
      <hr/>
        {data && (
                  <main className='grid grid-cols-4 gap-4'>
                  {data.map((e)=> (
                  <div>
                    <div key={Math.random} className="max-w-sm h-full bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                        <a href='' >
                            <img className="rounded-t-lg w-full h-40" src={e.urlToImage ? e.urlToImage : 'data:image/png;base64,'} alt="" />
                        </a>
                        <div className="p-5">
                            <a href="#">
                                <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
                                  {e.title.slice(0, 40)}
                                </h5>
                            </a>
                            <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">
                              {e.content.slice(0, 60)}
                            </p>
                            <a href={e.url} target='_blank' className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                Read more
                            </a>
                        </div>
                    </div>
                  </div>
                ))}
                </main>
        )}
    </div>
  )
}

export default App
